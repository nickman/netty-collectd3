 ...
 *
 */
package com.heliosapm.metrics.netty.collectd.util;
...
/**
 * <p>Title: URLHelper</p>
 * <p>Description: Static URL, File and URI helper methods.</p> 
 * <p>Company: Helios Development Group LLC</p>
 * @author Whitehead (nwhitehead AT heliosdev DOT org)
 * <p><code>com.heliosapm.metrics.netty.collectd.util.URLHelper</code></p>
 */
...
public class URLHelper {
	...
	/**
	 * Reads all lines from the resource pointed to by the passed URL.
	 * If the URL has kabillions of lines, you're on your own.
	 * @param url The URL to read the lines from
	 * @return an array of the read lines 
	 */
	public static String[] getLines(final URL url) {
		return getLines(url, 0);
	}
	
	/**
	 * Reads the first <b><code>n</code></b> lines from the resource pointed to by the passed URL
	 * @param url The URL to read the lines from
	 * @param linesToRead The number of lines to read or zero to read all
	...
	public static String[] getLines(final URL url, final int linesToRead) {
		if(url==null) throw new IllegalArgumentException("The passed URL was null");
		final int ltr = linesToRead==0 ? 128 : linesToRead;
		final int mtr = linesToRead==0 ? Integer.MAX_VALUE : linesToRead;
		if(ltr < 0) throw new IllegalArgumentException("Invalid number of lines [" + ltr + "]. Must be >= 0");
		InputStream is = null;
		BufferedReader br = null;
		InputStreamReader isr = null;
		URLConnection connection = null;		
		try {
			...
			br = new BufferedReader(isr);
			String line = null;
			final List<String> strLines = new ArrayList<String>(ltr);
			int linesRead = 0;
			while((line=br.readLine())!=null && linesRead < mtr) {
				strLines.add(line);
				linesRead++;
			}
			return strLines.toArray(new String[strLines.size()]);
		} catch (Exception ex) {
			throw new RuntimeException("Failed to read from [" + url + "]");
		} finally {
